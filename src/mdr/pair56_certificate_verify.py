...
from .go_bulk_verify import GoBulkReplay
from .pair56_certificates import PROOF_MAX_LENGTH, SCHEMA, file_sha256, verification_sha256
from .pair56_problem import full_state_to_pair56, load_problem
from .state import FullState, Move, in_target


DEFAULT_PROBLEM = ROOT / "training" / "pair56" / "problem.json"
DEFAULT_IDS = ROOT / "reductions" / "pair56" / "remaining_ids.bin"
DEFAULT_STATES = ROOT / "compositions" / "pair56" / "unique_states.bin"
...
def verify(
    *, database: Path, problem_path: Path, hard_ids_path: Path,
    states_path: Path, go_verifier: Path | None = None,
) -> dict[str, object]:
    ...
    if len(states_blob) % 108 or len(hard_ids_blob) % 4:
        raise ValueError("malformed pair56 certificate source artifacts")
    allowed_ids = {
    ...
    beams: dict[str, int] = {}
    go_replay = GoBulkReplay(
        go_verifier, target="g9", max_length=PROOF_MAX_LENGTH,
    ) if go_verifier is not None else None
    try:
        with closing(_open_checked_database(
            ...
            for (
                ...
                if previous_state_id is not None and state_id <= previous_state_id:
                    raise ValueError(f"duplicate or unsorted certificate state ID {state_id}")
                ...
                if (
                    len(solution) != length or length > PROOF_MAX_LENGTH
                    or any(move >= len(actions) for move in solution)
                ):
                    raise ValueError(f"invalid solution encoding at {state_id}")
                expected_verification = verification_sha256(
                ...
                if expected_verification != verification_digest:
                ...
                if not in_target(state.apply(word), "g9"):
                    raise ValueError(f"FullStateV1 replay failed at {state_id}")
                if go_replay is not None:
                    go_replay.feed(
                        state_id=state_id, state_bytes=state_bytes,
                        word=" ".join(word_names),
                    )
                verified += 1
                previous_state_id = state_id
                maximum = max(maximum, length)
                beams[str(beam_width)] = beams.get(str(beam_width), 0) + 1
        go_report = go_replay.finish(
            expected_count=verified,
            expected_maximum_length=maximum if verified else None,
        ) if go_replay is not None else None
    except BaseException:
        if go_replay is not None:
            go_replay.abort()
        raise
    if go_report is not None:
        go_report["verifier"] = _portable_path(go_verifier)
    return {
        "valid": True,
        "pair": "pair56",
        ...
        "replay": {
            "python_quotient": verified,
            "python_full_state": verified,
            "go_full_state": go_report,
        },
    }


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Independently replay pair56 solution certificates.")
    ...
    result.add_argument("--go-verifier", type=Path)
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        print(json.dumps(verify(
            ...
            states_path=args.composition_states.resolve(),
            go_verifier=args.go_verifier.resolve() if args.go_verifier is not None else None,
