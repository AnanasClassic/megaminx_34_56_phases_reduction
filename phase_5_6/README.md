# Phases 5+6: exhaustive certificate for `R_Sigma5(G7,G9) <= 25`
...
maximal-layer Cartesian product has `3,484 * 117 = 407,628` physical
states. Exact boundary-window rewriting over the first seven faces (the 28
FTM actions of `G5`) certifies 8,461 states without a model. The SQLite
database contains bounded words over the 20-move `G7` source alphabet for all
399,167 remaining states. The maximum independently replayed word length is
25. Because the exact rewrites may temporarily leave `G7`, the complete result
is a target bound over the 28-move `Sigma5` alphabet, not a claim that the
`G7/G9` source-alphabet radius over `Sigma7` is 25. Since `Sigma5` is contained
in the full FTM alphabet, the ambient bound `R_Omega(G7,G9) <= 25` follows.
...
streams the same 399,167 physical records to the separately implemented Go
